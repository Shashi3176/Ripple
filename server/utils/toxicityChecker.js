...
if (process.env.HUGGINGFACE_API_DNS) {
  const dnsServers = process.env.HUGGINGFACE_API_DNS.split(',').map((s) => s.trim()).filter(Boolean);
  if (dnsServers.length > 0) {
    dns.setServers(dnsServers);
  }
}

const API_URL = `https://router.huggingface.co/hf-inference/models/${config.model}`;
const REQUEST_TIMEOUT = 30000;
const MAX_RETRIES_503 = 2;
const MAX_RETRIES_NETWORK = 3;
const RETRY_DELAY_503_BASE = 10000;
const RETRY_DELAY_NETWORK_BASE = 2000;
const RETRY_DELAY_NETWORK_MAX = 16000;
...
//       console.log(`[ToxicityChecker] Model loading (503), retrying after ${delay / 1000}s (attempt ${retries + 1}/${MAX_RETRIES_503})`);
    ...
    if (categorized.type === 'network') {
      if (retries < MAX_RETRIES_NETWORK) {
        const backoff = Math.min(RETRY_DELAY_NETWORK_BASE * Math.pow(2, retries), RETRY_DELAY_NETWORK_MAX);
//         console.log(`[ToxicityChecker] Network error (${code || 'unknown'}), retrying after ${backoff / 1000}s (attempt ${retries + 1}/${MAX_RETRIES_NETWORK})`);
        await sleep(backoff);
        return makeApiRequest(messageText, retries + 1);
      }
      if (code === 'ENOTFOUND') {
//         console.error(`[ToxicityChecker] DNS resolution failed for ${API_URL} - check network/DNS settings`);
      }
    }
